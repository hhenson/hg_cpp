...
TEST_CASE("mutable map: the Mutable schema axis interns distinctly from the immutable map")
{
    ...
    CHECK(mutable_->kind == ValueTypeKind::Map);
...
TEST_CASE("mutable map: insert, lookup, contains, replace, and erase")
{
...

    {
        ...
        m.set_item(Value{1}.view(), Value{100}.view());
        m.set_item(Value{2}.view(), Value{200}.view());
        m.set_item(Value{3}.view(), Value{300}.view());
    }

    auto view = map.as_map();
    REQUIRE(view.size() == 3);
    CHECK(view.contains(Value{2}.view()));
    CHECK_FALSE(view.contains(Value{9}.view()));
    CHECK(view.at(Value{1}.view()).checked_as<int>() == 100);
    CHECK(view.at(Value{3}.view()).checked_as<int>() == 300);

    // Replace an existing key's value.
    map.as_map().begin_mutation().set_item(Value{2}.view(), Value{222}.view());
    CHECK(map.as_map().size() == 3);
    CHECK(map.as_map().at(Value{2}.view()).checked_as<int>() == 222);

    // Erase.
    CHECK(map.as_map().begin_mutation().remove(Value{2}.view()));
    CHECK_FALSE(map.as_map().begin_mutation().remove(Value{2}.view()));  // already gone
    CHECK(map.as_map().size() == 2);
    CHECK_FALSE(map.as_map().contains(Value{2}.view()));
    CHECK(map.as_map().at(Value{1}.view()).checked_as<int>() == 100);
}
...
TEST_CASE("mutable map: iteration over keys, values and entries; clear")
{
    ...
    {
        auto m = map.as_map().begin_mutation();
        m.set_item(Value{1}.view(), Value{10}.view());
        m.set_item(Value{2}.view(), Value{20}.view());
        m.set_item(Value{3}.view(), Value{30}.view());
    }
    ...
    {
        key_sum += key.checked_as<int>();
        val_sum += value.checked_as<int>();
    }
...
TEST_CASE("mutable map: equality and a copy is independent")
{
    ...
    b.as_map().begin_mutation().set_item(Value{3}.view(), Value{30}.view());
...
TEST_CASE("mutable map: string -> Any (GlobalState shape) holds heterogeneous values")
{
    ...
    Value dict = make_mutable_map(str_meta, any_meta);
    {
        ...
        m.set_item(Value{std::string{"count"}}.view(), make_any(Value{42}).view());
        m.set_item(Value{std::string{"ratio"}}.view(), make_any(Value{1.5}).view());
    }

    auto view = dict.as_map();
    REQUIRE(view.size() == 2);
    CHECK(view.at(Value{std::string{"count"}}.view()).as_any().get().checked_as<int>() == 42);
