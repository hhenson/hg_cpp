...
namespace hgraph
{
    void TimeSeriesType::register_with_nanobind(nb::module_ &m) {
        nb::class_<TimeSeriesType>(
            ...
            .def_prop_ro("owning_node", &TimeSeriesType::owning_node)
            .def_prop_ro("owning_graph", &TimeSeriesType::owning_graph)
    ...
    Node::ptr TimeSeriesOutput::owning_node() const {
        if (_parent_output_or_node.has_value()) {
            ...
            throw std::runtime_error("No node is accessable");
        }
    }
    ...
    void TimeSeriesOutput::re_parent(Node::ptr parent) {
