...
namespace hgraph
{
    namespace
    {
        ...
        {
            virtual ~TargetLinkContext() = default;
        ...
        struct TargetLinkContextFor final : TargetLinkContext
        {
            Layout layout{};
            Ops    ops{};
        };

        struct TargetLinkDictContext final : TargetLinkContext
        {
        ...
        {
            return static_cast<const TargetLinkContext *>(context)->active_layout;
        }
        ...
        [[nodiscard]] ValueView target_link_set_key_projector(const void *context,
                                                              ...
                                                              std::size_t slot)
        {
            const auto *state = static_cast<const TargetLinkContext *>(context);
        ...
        [[nodiscard]] std::size_t target_link_indexed_size(const void *context, const void *memory) noexcept
        {
            return fallback_on_exception(std::size_t{0}, [&] {
                auto target = target_link_target_view(context, memory);
                if (!target.valid()) { return std::size_t{0}; }
                const auto *schema = static_cast<const TargetLinkContext *>(context)->schema;
        ...
        [[nodiscard]] TSDataOps target_link_base_ops(TargetLinkContext &context)
        {
            return TSDataOps{
                .context                   = &context,
        ...
        [[nodiscard]] std::unique_ptr<TargetLinkContext>
        ...
        using TargetLinkContextBuilder = std::unique_ptr<TargetLinkContext> (*)(
            const TSValueTypeMetaData &schema,
            const MemoryUtils::StoragePlan &root_plan,
            std::size_t storage_offset,
            const TSDataBinding &regular_binding,
        ...
        [[nodiscard]] const TargetLinkContextBuilder &target_link_context_builder_for(TSTypeKind kind)
        {
            static constexpr std::size_t kind_count = ts_kind_index(TSTypeKind::SIGNAL) + 1U;
            static const std::array<TargetLinkContextBuilder, kind_count> table{
                                                                    ...
                                                                    std::size_t                       storage_offset)
        {
            ...
            auto context = target_link_context_builder_for(schema->kind)(*schema, root_plan, storage_offset,
                                                                         *regular_binding, *regular_layout);
