...
namespace hgraph::stdlib
{
    ...
    /** ``ne_`` — the ``!=`` operator. */
    struct ne_ : Operator<"ne", In<"lhs", TsVar<"S">>, In<"rhs", TsVar<"S">>, Out<TS<Bool>>>
    {
    };

    /** ``lt_`` — the ``<`` operator. */
    struct lt_ : Operator<"lt", In<"lhs", TsVar<"S">>, In<"rhs", TsVar<"S">>, Out<TS<Bool>>>
    {
    };

    /** ``le_`` — the ``<=`` operator. */
    struct le_ : Operator<"le", In<"lhs", TsVar<"S">>, In<"rhs", TsVar<"S">>, Out<TS<Bool>>>
    {
    };

    /** ``gt_`` — the ``>`` operator. */
    struct gt_ : Operator<"gt", In<"lhs", TsVar<"S">>, In<"rhs", TsVar<"S">>, Out<TS<Bool>>>
    {
    };

    /** ``ge_`` — the ``>=`` operator. */
    struct ge_ : Operator<"ge", In<"lhs", TsVar<"S">>, In<"rhs", TsVar<"S">>, Out<TS<Bool>>>
    {
    };

    /** ``cmp_`` — three-way comparison; returns ``LT`` / ``EQ`` / ``GT`` in one step. */
    struct cmp_ : Operator<"cmp", In<"lhs", TsVar<"S">>, In<"rhs", TsVar<"S">>, Out<TS<CmpResult>>>
    {
    };
    ...
    struct min_ : Operator<"min", In<"ts", TsVar<"S">>, Out<TsVar<"O">>>
    {
    };

    /** ``max_`` — the ``max`` operator. Variadic, mirroring ``min_``. */
    struct max_ : Operator<"max", In<"ts", TsVar<"S">>, Out<TsVar<"O">>>
    {
    };
}  // namespace hgraph::stdlib
