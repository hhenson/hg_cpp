...
namespace
{
    ...
    // TS<std::int32_t> -> TSL<TS<std::int32_t>, 2>: child 0 = value, child 1 = value * 10. Exercises both
    // Out<TSL> forms — the flat `set(i, v)` and the per-child `out[i].set(v)`.
    struct Spread
    {
        static constexpr auto name = "tsl_spread";
        static void           eval(In<"in", TS<std::int32_t>> in, Out<TSL<TS<std::int32_t>, 2>> out)
        {
            out.set(0, in.value());
    ...
    // TSL<TS<std::int32_t>, 2> -> TS<std::int32_t>: sum of the two children's current values (recursive
    // child selectors: l[i] is an In<"", TS<std::int32_t>>).
    struct Total
    {
        static constexpr auto name = "tsl_total";
        static void           eval(In<"l", TSL<TS<std::int32_t>, 2>> l, Out<TS<std::int32_t>> out) { out.set(l[0].value() + l[1].value()); }
    };

    // TSL<TS<std::int32_t>, 2> -> TS<std::int32_t>: number of children that ticked this cycle (the size
    // of the canonical delta map).
    struct ModifiedCount
    {
        static constexpr auto name = "tsl_modified_count";
        static void           eval(In<"l", TSL<TS<std::int32_t>, 2>> l, Out<TS<std::int32_t>> out)
        {
            out.set(static_cast<std::int32_t>(l.delta().as_map().size()));
        }
    };

    // TSL<TS<std::int32_t>> -> TSL<TS<std::int32_t>>: re-tick each modified child, so the output delta
    // mirrors the input.
    struct MirrorList
    {
        static constexpr auto name = "tsl_mirror";
        static void           eval(In<"l", TSL<TS<std::int32_t>, 2>> l, Out<TSL<TS<std::int32_t>, 2>> out)
        {
            for (auto &&[index, child] : l.modified_items()) { out.set(index, child.value().template checked_as<std::int32_t>()); }
        }
    };

    // TS<std::int32_t> -> dynamic TSL<TS<std::int32_t>>: index 0 exists from the first tick and index 1
    ...
    struct DynamicSpread
    {
        ...
        static void           eval(In<"in", TS<std::int32_t>> in, Out<TSL<TS<std::int32_t>>> out)
        {
            out[0].set(in.value());
    ...
    // dynamic TSL<TS<std::int32_t>> -> TS<std::int32_t>: reads the live target-bound size and sums the
    // currently valid child values.
    struct DynamicTotal
    {
        static constexpr auto name = "dynamic_tsl_total";
        static void           eval(In<"l", TSL<TS<std::int32_t>>> l, Out<TS<std::int32_t>> out)
        {
            std::int32_t total = 0;
    ...
    struct DynamicMirrorList
    {
        ...
        static void           eval(In<"l", TSL<TS<std::int32_t>>> l, Out<TSL<TS<std::int32_t>>> out)
        {
            for (auto &&[index, child] : l.modified_items()) { out.set(index, child.value().template checked_as<std::int32_t>()); }
        }
    };
    ...
    struct SpreadGraph
    {
        ...
        static void           compose(Wiring &w)
        {
            ...
            auto sp  = wire<Spread>(w, src);   // -> Port<TSL<TS<std::int32_t>, 2>>
    ...
    struct ListDeltaGraph
    {
        ...
        static void           compose(Wiring &w)
        {
            auto src = wire<testing::replay, TSL<TS<std::int32_t>, 2>>(w, std::string{"in"});
    ...
    struct DynamicSpreadGraph
    {
        ...
        {
            auto src = wire<testing::replay, TS<std::int32_t>>(w, std::string{"in"});
            auto sp  = wire<DynamicSpread>(w, src);  // -> Port<TSL<TS<std::int32_t>>>
    ...
    struct DynamicListDeltaGraph
    {
        ...
        static void           compose(Wiring &w)
        {
            auto src = wire<testing::replay, TSL<TS<std::int32_t>>>(w, std::string{"in"});
            wire<testing::record>(w, src, std::string{"out"});
        }
    };
    ...
    struct ListSetDeltaGraph
    {
        ...
        static void           compose(Wiring &w)
        {
            auto src = wire<testing::replay, TSL<TSS<std::int32_t>, 2>>(w, std::string{"in"});
            wire<testing::record>(w, src, std::string{"out"});
        }
    };
}  // namespace
...
TEST_CASE("tsl: list_delta map and positional forms agree (canonical Value)")
{
    ...
    const Value a = list_delta<TS<std::int32_t>>({{0, 10}, {2, 30}});
    const Value b = list_delta<TS<std::int32_t>>({10, none, 30});  // positional: index 1 skipped
    CHECK(a.equals(b));
    ...
    CHECK(map.at(Value{std::int64_t{2}}.view()).checked_as<std::int32_t>() == 30);

    CHECK_FALSE(a.equals(list_delta<TS<std::int32_t>>({{0, 10}, {2, 31}})));  // different value
}
...
TEST_CASE("tsl: replay<TSL> -> record<TSL> round-trips list deltas (modified children only)")
{
    ...
    const std::vector<std::optional<Value>> deltas{
        ...
        list_delta<TS<std::int32_t>>({{1, 9}}),            // only child 1 ticks
    };
...
TEST_CASE("tsl: eval_node drives scalar-child TSL inputs/outputs as canonical deltas")
{
    ...
    CHECK_OUTPUT(testing::eval_node<Spread>({1, 2}),
                 {list_delta<TS<std::int32_t>>({{0, 1}, {1, 10}}), list_delta<TS<std::int32_t>>({{0, 2}, {1, 20}})});

    // TSL -> TS: sum of children (child 1 persists across the second cycle).
    const std::vector<std::optional<Value>> in{list_delta<TS<std::int32_t>>({{0, 1}, {1, 2}}), list_delta<TS<std::int32_t>>({{0, 5}})};
    ...
    const std::vector<std::optional<Value>> dynamic_in{list_delta<TS<std::int32_t>>({{0, 1}}),
                                                       list_delta<TS<std::int32_t>>({{0, 5}, {1, 9}}),
                                                       list_delta<TS<std::int32_t>>({{1, 11}})};
...
TEST_CASE("tsl: recursive — list_delta over container children builds nested canonical Values")
{
    ...
    // TSL<TSS<std::int32_t>> delta: Map<int, Bundle{added:Set, removed:Set}> — set_delta nested
    ...
    CHECK_FALSE(sd.equals(list_delta<TSS<std::int32_t>>({{0, set_delta<std::int32_t>({1, 2}, {})}})));
    {
    ...
    // TSL<TSL<TS<std::int32_t>,2>> delta: Map<int, Map<int, int>> — list_delta nested in list_delta.
    const Value nd = list_delta<TSL<TS<std::int32_t>, 2>>({{0, list_delta<TS<std::int32_t>>({{0, 7}, {1, 8}})}});
    CHECK(nd.equals(list_delta<TSL<TS<std::int32_t>, 2>>({{0, list_delta<TS<std::int32_t>>({{1, 8}, {0, 7}})}})));
...
TEST_CASE("tsl: a TSL<TSS> executes end-to-end (replay -> record round-trips list-of-set deltas)")
{
    ...
    const std::vector<std::optional<Value>> deltas{
        list_delta<TSS<std::int32_t>>({{0, set_delta<std::int32_t>({1, 2}, {})}, {1, set_delta<std::int32_t>({9}, {})}}),
        list_delta<TSS<std::int32_t>>({{1, set_delta<std::int32_t>({8}, {9})}}),
        list_delta<TSS<std::int32_t>>({{0, set_delta<std::int32_t>({3}, {1})}}),
    };
